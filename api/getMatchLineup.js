...
export default async function handler(request, response) {
  ...
  const apiUrl = `https://www.thesportsdb.com/api/v1/json/${apiKey}/lookuplineup.php?id=${id}`;
  ...
  try {
    ...
    if (!data.lineup) {
      ...
      throw new Error('La risposta dell\'API non contiene le formazioni.');
    }
    ...
    console.error("Errore nella funzione serverless 'getMatchLineup':", error);
